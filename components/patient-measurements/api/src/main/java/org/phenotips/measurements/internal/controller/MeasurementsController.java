...
public class MeasurementsController implements PatientDataController<MeasurementEntry> {
	...
	/**
	 * Parses string representations of document references into proper
	 * references.
	 */
	@Inject
	@Named("current")
		...
		try {
			...
			for (BaseObject object : objects) {
				...
				try {
					...
					// getting measurement units, could be null, but should not
					// be
	...
	private Map<String, Object> computeDependantInfo(MeasurementEntry entry, Patient patient,
		...
		if (sexData != null) {
			...
			if (StringUtils.isNotBlank(sex)) {
				// ideally, the constants from the SexController should be used
				// here,
				// but that would introduce a hard dependency. Maybe add it to
				// constants?
				if (!entry.getType().equals(ARMSPAN)
						&& (StringUtils.equals(sex, maleConst) || StringUtils.equals(sex, femaleConst))) {
					// has to be either male or female, otherwise how can we
					// compute SD?
					// finding the handler
					...
					result.put(SD, Double.isFinite(sd) ? sd : "");
